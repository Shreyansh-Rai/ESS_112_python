def matmul(m1,m2):
	...
	q=len(m2[0]) #number of columns in m2
	c=len(m2) #the number of rows in m2
	if len(m2)!=len(m1[0]) : 
		return "The matrices cannot be multiplied"
	res= [[0 for j in range(q)] for i in range(p)] #creating a result matrix of pXq

	for i in range(p) :
		for j in range(q):
			for k in range(c):
				res[i][j]+= m1[i][k]*m2[k][j] #calculating the sum
	return res
